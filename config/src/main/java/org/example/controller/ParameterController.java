...
import org.example.bean.Monster02;
...
//                               用map id是key name是value 这样就不用一个一个pathVariable获取了
                               @PathVariable Map<String, String> map) {
        System.out.println("id-" + id);
        System.out.println("name-" + name);
        System.out.println("map-" + map);//map-{id=1, name=me}
        return "success";
    }
...
//    post也行
     ...
     *  required = false 获取不到不报错 不然拿不到后端要报错
     */
    @GetMapping("/cookie")
     ...
     */
    @PostMapping("/save")
//    拿到的是一连串 name=123&age=123这种格式的字符串
    ...
    public String saveMonster(Monster02 monster02) {
        System.out.println("monster-" + monster02);
        return "success";
    }
